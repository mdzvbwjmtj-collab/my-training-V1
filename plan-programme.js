...
const renderOverview=()=>{
 ...
 app.innerHTML=`<main class="shell programme-page">
  ...
  <section class="programme-section upcoming-section">
   ...
   <button class="phase-card phase-card-empty" type="button" onclick="return false" aria-disabled="true">
...
const showPlan=()=>setTimeout(renderOverview,0);
const install=()=>{
 const g=window.go;
 if(typeof g==='function'&&!g.__programmePlan){
  const wrapped=s=>{g(s);if(s==='plan')showPlan()};
  wrapped.__programmePlan=true;
  window.go=wrapped;
 }
 try{if(new URLSearchParams(location.search).get('screen')==='plan'||JSON.parse(localStorage.getItem('myTrainingDashboard')||'{}').screen==='plan')showPlan()}catch(e){}
};
document.addEventListener('click',e=>{
 const b=e.target.closest('button');
 if(b&&/go\(['"]plan['"]\)/.test(b.getAttribute('onclick')||''))showPlan();
},true);
