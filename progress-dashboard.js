...
function chart(all){
 ...
 for(let i=0;i<tickCount;i++){
   const d=new Date(startMs+(span*i/(tickCount-1)));
   const day=d.getDate(),month=d.toLocaleDateString('en-GB',{month:'short'});
   const xx=L+(i/(tickCount-1))*pw;
   const anchor=i===0?'start':i===tickCount-1?'end':'middle';
   dateTicks.push(`<line x1="${xx}" x2="${xx}" y1="${H-B}" y2="${H-B+1.5}" stroke="#777" stroke-width="0.4"/><text x="${xx}" y="${H-4}" fill="#888" font-size="2.9" text-anchor="${anchor}">${day} ${month}</text>`);
 }
...
function nav(){return `<nav class="nav"><button onclick="window.render()">Today</button><button onclick="window.plan()">Plan</button><button class="active" onclick="window.progressDashboard()">Progress</button></nav>`}
function dashboard(){
 ...
 const trend=delta==null?'—':(delta>0?'+':'')+delta.toFixed(1)+' kg';
