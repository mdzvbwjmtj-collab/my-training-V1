...
function render(){
 ...
 if(!card){
  card=document.createElement('div');card.id='weight-chart-card';card.className='card';
 ...
 const shown=all.filter(d=>new Date(d[0]+'T00:00:00')>=cut);
 const pts=shown.length?shown:all;
 const vals=pts.map(d=>d[1]),min=Math.floor(Math.min(...vals)-.5),max=Math.ceil(Math.max(...vals)+.5),W=720,H=300,L=55,R=18,T=30,B=55;
 ...
 card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div><div class="eyebrow">BODYWEIGHT</div><h3 style="margin:3px 0 0">Weight trend</h3></div><div style="display:flex;gap:6px"><button class="chart-range ${range===1?'selected':''}" data-range="1">1 month</button><button class="chart-range ${range===3?'selected':''}" data-range="3">3 months</button><button class="chart-range ${range===6?'selected':''}" data-range="6">6 months</button></div></div><div style="margin-top:10px;color:#999;font-size:12px">${pts.length} weigh-ins · kg</div><div style="width:100%;overflow:hidden;margin-top:4px"><svg viewBox="0 0 ${W} ${H}" width="100%" height="300" role="img" aria-label="Bodyweight trend">${grid}<polyline points="${points}" fill="none" stroke="#fff" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>${marks}${labels}</svg></div>`;
 card.querySelectorAll('.chart-range').forEach(b=>b.onclick=()=>{range=Number(b.dataset.range);render()});
}
