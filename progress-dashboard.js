...
function render(){
 ...
 card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div><div class="eyebrow">BODYWEIGHT</div><h3 style="margin:3px 0 0">Weight trend</h3></div><div style="display:flex;gap:6px"><button type="button" class="chart-range ${range===1?'selected':''}" data-range="1">1 month</button><button type="button" class="chart-range ${range===3?'selected':''}" data-range="3">3 months</button><button type="button" class="chart-range ${range===6?'selected':''}" data-range="6">6 months</button></div></div><div style="margin-top:10px;color:#999;font-size:12px">${pts.length} weigh-ins · kg</div><div style="width:100%;overflow:hidden;margin-top:4px"><svg viewBox="0 0 ${W} ${H}" width="100%" height="300" role="img" aria-label="Bodyweight trend">${grid}<polyline points="${points}" fill="none" stroke="#fff" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>${marks}${labels}</svg></div>`;
 rendering=false;
}
document.addEventListener('click',e=>{const b=e.target.closest?.('.chart-range');if(!b)return;e.preventDefault();range=Number(b.dataset.range);render()});
