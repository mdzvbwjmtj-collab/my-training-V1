...
function draw(force=false){
 ...
 const marks=pts.map(d=>`<g><circle cx="${x(d[0])}" cy="${y(d[1])}" r="5" fill="currentColor"/><text x="${x(d[0])}" y="${y(d[1])-11}" fill="currentColor" font-size="11" text-anchor="middle">${d[1].toFixed(1)}</text></g>`).join('');
 const labels=`<text x="${L}" y="${H-18}" fill="currentColor" opacity=".62" font-size="10" text-anchor="start">${start.toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</text><text x="${W-R}" y="${H-18}" fill="currentColor" opacity=".62" font-size="10" text-anchor="end">${now.toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</text>`;
 card.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div><div class="eyebrow">BODYWEIGHT</div><h3 style="margin:3px 0 0">Weight trend</h3></div><div style="display:flex;gap:6px;position:relative;z-index:2"><button type="button" class="chart-range ${range===1?'selected':''}" onclick="window.setWeightRange(1);return false">1 month</button><button type="button" class="chart-range ${range===3?'selected':''}" onclick="window.setWeightRange(3);return false">3 months</button><button type="button" class="chart-range ${range===6?'selected':''}" onclick="window.setWeightRange(6);return false">6 months</button></div></div><div style="margin-top:10px;color:inherit;opacity:.65;font-size:12px">${pts.length} weigh-ins · ${range}-month view</div><div style="width:100%;overflow:hidden;margin-top:4px;color:inherit"><svg viewBox="0 0 ${W} ${H}" width="100%" height="300" role="img" aria-label="Bodyweight trend">${grid}<polyline points="${points}" fill="none" stroke="currentColor" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/>${marks}${labels}</svg></div>`;
}
window.setWeightRange=function(n){range=Number(n);draw(true)};
function ensure(){const main=document.querySelector('main.shell');if(main?.querySelector('.top .title')?.textContent.trim()==='Your progress')draw()}
setInterval(ensure,500);
setTimeout(()=>draw(true),50);
})();