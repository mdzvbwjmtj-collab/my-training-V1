...
function restTick(){if(!restEndsAt)return;const left=Math.max(0,(restEndsAt-Date.now())/1000);if(left<=0){restEndsAt=0;if(restInterval){clearInterval(restInterval);restInterval=null}pingRest();render();return}render()}
function startRest(seconds=restDuration){const s=restOptions.includes(Number(seconds))?Number(seconds):restDuration;restDuration=s;restEndsAt=Date.now()+s*1000;if(restInterval)clearInterval(restInterval);restInterval=setInterval(restTick,1000);render();}
function stopRest(){restEndsAt=0;if(restInterval){clearInterval(restInterval);restInterval=null}render()}
...
window.toggle=function(ei,si){baseToggleWithTimer(ei,si);const l=currentLog();if(l?.sets?.[ei]?.[si]?.completed)startRest(restDuration)};
