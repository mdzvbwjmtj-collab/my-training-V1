...
function startRest(seconds=restDuration){const s=restOptions.includes(Number(seconds))?Number(seconds):restDuration;restDuration=s;restEndsAt=Date.now()+s*1000;if(restInterval)clearInterval(restInterval);restInterval=setInterval(restTick,250);render();}
