...
function restTick(){if(!restEndsAt)return;const left=Math.max(0,(restEndsAt-Date.now())/1000);if(left<=0){restEndsAt=0;if(restInterval){clearInterval(restInterval);restInterval=null}pingRest();if(restNext) {openSets[restNext.key]=true;restNext=null}render();return}render()}
let restNext=null;
function startRest(seconds=restDuration,nextKey=null){const s=restOptions.includes(Number(seconds))?Number(seconds):restDuration;restDuration=s;restNext=nextKey?{key:nextKey}:null;restEndsAt=Date.now()+s*1000;if(restInterval)clearInterval(restInterval);restInterval=setInterval(restTick,1000);render()}
...
function setOpenKey(ei,si){return `${currentWorkout().id}:${ei}:${si}`}
function collapsibleSets(){const w=currentWorkout(),l=currentLog();return w.exercises.map((e,ei)=>{const targets=reps(e),cur=l.sets[ei]||{};return targets.map((target,si)=>{const d=cur[si]||{},open=si===0||openSets[setOpenKey(ei,si)]||Boolean(d.completed);return `<div class="collapsible-set ${open?'open':''}"><button class="set-summary" onclick="toggleSetOpen(${ei},${si})"><span>Set ${si+1}</span><span>${d.completed?'✓ Completed':open?'Open':'Locked'}</span></button>${open?`<div class="set-content"><span>Target: ${target} reps</span><span>${d.weight??''} kg · ${d.reps??''} reps</span></div>`:''}</div>`}).join('')}).join('')}
function toggleSetOpen(ei,si){const k=setOpenKey(ei,si);openSets[k]=!openSets[k];render()}
const baseWorkoutWithTimer=window.workout;
window.workout=function(){let h=baseWorkoutWithTimer();return h.replace('<div class="card hero">',restTimer()+'<div class="card hero">')};
const baseToggleWithTimer=window.toggle;
window.toggle=function(ei,si){baseToggleWithTimer(ei,si);const l=currentLog();if(l?.sets?.[ei]?.[si]?.completed){const targets=reps(currentWorkout().exercises[ei]);const nextSi=si+1;const nextKey=nextSi<targets.length?setOpenKey(ei,nextSi):null;startRest(restDuration,nextKey)}};
