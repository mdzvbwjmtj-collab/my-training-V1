...
function restTick(){if(!restEndsAt)return;const left=Math.max(0,(restEndsAt-Date.now())/1000);if(left<=0){const next=restNext?.key||null;restEndsAt=0;if(restInterval){clearInterval(restInterval);restInterval=null}pingRest();restNext=null;render();setTimeout(()=>{if(next)document.dispatchEvent(new CustomEvent('rest-complete',{detail:{key:next}}))},0);return}render()}
let restNext=null;
...
window.toggle=function(ei,si){baseToggleWithTimer(ei,si);const l=currentLog();if(l?.sets?.[ei]?.[si]?.completed){const targets=reps(currentWorkout().exercises[ei]);const nextSi=si+1;const nextKey=nextSi<targets.length?`${currentWorkout().id}:${ei}:${nextSi}`:null;startRest(restDuration,nextKey)}};
