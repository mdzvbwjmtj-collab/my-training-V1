(function(){
 ...
 function build(card,ei){
  ...
  const actualEi=Number(card.closest('.exercise-page')?.dataset.exerciseIndex??ei);
  rows.forEach((row,si)=>{
   const k=key(actualEi,si);const check=row.querySelector('.check');const done=!!(check&&check.classList.contains('checked'));
  ...
  head.after(flow);card.dataset.setFlowReady='1';
 }
