...
const enhance=()=>{
  ...
  const apply=()=>{items.forEach((row,i)=>row.style.display=open||i<7?'':'none');toggle.textContent=open?'Hide older entries':'Show full history'};
...
if(typeof originalRender==='function'){
  window.render=function(){originalRender();requestAnimationFrame(enhance)};
}
